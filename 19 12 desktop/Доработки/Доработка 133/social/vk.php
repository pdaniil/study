...
try {	
	...
	$link_auth = 'https://oauth.vk.com/authorize?client_id='. $client_id .'&display=page&redirect_uri='. $redirect_uri .'&scope=email,offline&response_type=code&v=5.131';
	if (!isset($_GET["code"]))
	{
	...
	$link_get_token = 'https://oauth.vk.com/access_token?client_id='. $client_id .'&client_secret='. $client_secret .'&redirect_uri='. $redirect_uri .'&code='. $_GET["code"];
	...
	if ($result != false)
	{
		...
		else
		{
			...
			<form method="POST" action="<?php echo "https://".$_SERVER['SERVER_NAME'];?>" id="form">
				...
				<input type="hidden" name="social_name" value="vk.com">
	...
	else
	{
		...
		$link_request = 'https://api.vk.com/method/users.get?user_ids='. $user_id .'&fields=bdate,contacts&access_token='. $token .'&v=5.131';
	
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
		curl_setopt($ch, CURLOPT_URL, $link_request);
		$result = curl_exec($ch);
		curl_close($ch);
		$obj= json_decode($result);
		if ($obj->error != null)
		{
			throw new Exception("Не удалось получить данные пользователя при регистрации.");
		}
		...
		<form method="POST" action="register.php" id="form">
			...
			<input type="hidden" name="name" value="<?php echo $response->first_name;?>">
			<input type="hidden" name="surname" value="<?php echo $response->last_name;?>">
			<input type="hidden" name="token" value="<?php echo $token; ?>">
			<input type="hidden" name="email" value="<?php echo $email; ?>">
			<input type="hidden" name="social_name" value="vk.com">
			<input type="hidden" name="reg_variant" value="1">
			<input type="hidden" name="uri_redirect" value="<?php echo "/content/users/social/vk.php";?>"><!-- Нужно для редиректа на страницу авторизации сразу после регистрации. -->
		</form>
